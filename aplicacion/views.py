...
def maps_appV1(request):
    ...
    colormap = cm.LinearColormap(colors=[ 'red', 'orange', 'yellow', '#2bfc00'],
                             index=[-120, -110, -95, -50], vmin=-120, vmax=-50,
                             caption='Gradiente de la caliadad para la señal (dBm)')
    ...
    for each in datos_csv.iterrows():
        ...
        blue = Color("red")
        colors = list(blue.range_to(Color("#2bfc00"),30))
...
def tower_view(request):
    ...
    for each in trres.iterrows():
        marker_info="CellID:%s\n LAC:%s\n Red:%s\n Tecnologia:%s\n Cobertura(m):%s"%(each[1]['cellid'],each[1]['lac'],each[1]['operador'],each[1]['tecnologia'],each[1]['cobertura'])
        tower_icon = folium.CustomIcon(
        icon_url,
        icon_size=(14,14), 
        icon_anchor=None, 
        ...
        html_popup= """
            ...
            """%(each[1]['cellid'],each[1]['lac'],each[1]['operador'],each[1]['tecnologia'],each[1]['cobertura'],each[1]['id'])
        popup=Popup(html=html_popup,max_width=500)
        folium.Marker(list((each[1]['latitud'],each[1]['longitud'])), 
                popup=popup,
                icon = tower_icon
